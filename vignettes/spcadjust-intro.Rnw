...
Then we generate a data set of past observations and compute
the resulting estimates for running the chart.
<<>>=
X <-  rnorm(250)
...
and the in-control variance by
$\hat \sigma^2=\frac{1}{n-1}\sum_{i=-n}^{-1} (X_i-\hat \mu)^2$.
Based on new observations $X_1,X_2,\dots$, a two-sided chart is then defined by
$$
...
Suppose one observes past data $(Y_{-n},X_{-n}),\ldots,(Y_{-1},X_{-1})$,
where $Y_i$ is a response of interest and $X_i$ is a corresponding
vector of covariates.
The parameter $\beta$ of the linear model $\E{Y}=X\beta$ is estimated via the
function lm (which does least squares estimation) and gives an estimator
$\hat \beta$.
...
@
Defining a chart for a specific linear model and finding the
...
Finally, running the chart with new data (that are out of control from
observation 50) and plotting the chart.
...
a CUSUM chart  can be defined  based on the cumulative sum of likelihood ratios of the
in-control versus out-of-control model  \citep{Steiner2000Msp} by
...
class that has the methods marked in blue in the following diagram.\\
\includegraphics[width=\textwidth]{Methods.pdf}


\section{Class structure}
The following slightly simplified UML diagram gives an overview over
